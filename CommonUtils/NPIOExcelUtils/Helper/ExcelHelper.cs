using CommonUtils.NPIOExcelUtils.Model;
using NPOI.HPSF;
...
namespace CommonUtils.NPIOExcelUtils.Helper
{
    /// <summary>
    ...
    public class ExcelHelper
    {
        ...
        public static MemoryStream ExportMemoryStream(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            //dtSource = dtSource.DefaultView.ToTable(true, arrCol);//去重,数据多时非常耗时
            dtSource = new DataView(dtSource).ToTable(false, arrCol); //不去重
            //筛选出配置和数据源都存在的列
...

            {
                ...
                si.ApplicationName = "CCTC"; //填加xls文件创建程序信息
